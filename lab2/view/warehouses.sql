CREATE OR REPLACE VIEW vw_warehouse_utilization_activity AS WITH stock_agg AS
    (SELECT s.warehouse_id,
            COALESCE(SUM(s.quantity), 0) AS total_units,
            COALESCE(SUM(s.quantity * COALESCE(p.volume_m3, 0)), 0)::numeric(14, 4) AS occupied_m3,
            COUNT(*) FILTER (
                             WHERE s.quantity < s.min_balance) AS below_min_cnt
     FROM stock s
...
SELECT w.id AS warehouse_id,
       ...
       CASE
           ...
           ELSE (COALESCE(sa.occupied_m3, 0) / w.capacity_m3 * 100)::numeric(7, 2)
       END AS fill_percent,
...
SELECT *
FROM vw_warehouse_utilization_activity;